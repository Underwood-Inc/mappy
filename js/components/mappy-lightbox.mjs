...
class MappyLightbox extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.#onClick = (e) => {
      ...
      // Open on data-lightbox image click (or click inside a known frame wrapper)
      const frame = target.closest(
        '.showcase__shot, .hero__visual, .screenshot-card__frame, .landing-section__figure',
      );
      const img =
        target.closest('[data-lightbox]') ?? frame?.querySelector('[data-lightbox]');
